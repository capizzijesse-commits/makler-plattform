...
export default function DirectPublicationDockV2({
  listingId,
}: {
  listingId: string;
}) {
  ...
  const [listing, setListing] =
    useState<ListingSnapshot | null>(null);
  ...
  const [cameFromDashboardSave, setCameFromDashboardSave] =
    useState(false);
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-300">
                  Objektdaten, Bilder und Inserattext reichen für diesen Automationsweg. Bewertung, Auftrag und CRM-Schritte bleiben optional.
                </p>
              </div>
            ...
            {!loading && !directReady ? (
              <div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[0.05] p-4 sm:p-5">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-3">
                  {[
                    { label: "Objektdaten", ready: readiness.core },
                    { label: "Bilder", ready: readiness.images },
                    { label: "Inserattext", ready: readiness.text },
                  ].map((item) => (
                    <div
                      key={item.label}
                ...
                <Link
                  href={`/cockpit/${encodeURIComponent(listingId)}/edit`}
                  onClick={() => setOpen(false)}
