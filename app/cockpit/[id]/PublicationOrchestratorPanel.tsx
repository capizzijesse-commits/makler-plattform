...
type PortalExternalReadiness = {
  status: PortalExternalReadinessStatus;
  controlledNetworkTestAllowed: boolean;
...
type PortalSnapshot = {
  ...
  externalReadiness?: PortalExternalReadiness | null;
};
...
type PublicationCapabilities = {
  ...
  orchestratorDispatchEnabled: boolean;
  portalQueueEnabled: boolean;
  canDispatchPortals: boolean;
  socialAutomaticDispatch: boolean;
};
...
type Props = {
  listingId: string;
  market: Market;
  onPublicationStateChange?: (state: {
    startedAt: string | null;
    publishedAt: string | null;
  }) => void;
};

const EMPTY_SUMMARY: PublicationSummary = {
  total: 0,
  pending: 0,
  publishing: 0,
  published: 0,
...
const DEFAULT_CAPABILITIES: PublicationCapabilities = {
  planEligible: false,
  orchestratorDispatchEnabled: false,
  portalQueueEnabled: false,
  canDispatchPortals: false,
  socialAutomaticDispatch: false,
};
...
function isPortalReadyForControlledTest(portal: PortalSnapshot): boolean {
  const external = portal.externalReadiness;

  return (
    portal.status === "verified" &&
    portal.environment === "test" &&
    external?.status === "READY_FOR_CONTROLLED_TEST" &&
    external.controlledNetworkTestAllowed === true &&
    external.productionEnabled === false &&
    external.networkAttempted === false
  );
}

function portalReadinessLabel(portal: PortalSnapshot): string {
  switch (portal.externalReadiness?.status) {
    ...
    case "READY_FOR_CONTROLLED_TEST":
      return isPortalReadyForControlledTest(portal)
        ? "Bereit für Test"
        : "Verbindung prüfen";
    default:
      return "Nicht bereit";
...
function portalReadinessDetails(portal: PortalSnapshot): string | undefined {
  const requirements = portal.externalReadiness?.missingRequirements;
...
function socialLabel(connection: SocialConnection) {
  const account =
  ...
  switch (connection.channel) {
    ...
    default:
      return connection.channel + " · " + account;
  }
}

function destinationLabel(target: PublicationTarget) {
  if (target.kind === "portal") {
    return portalLabel(target.destination);
  }
...
function statusClass(status: string) {
  ...
  if (status === "publishing" || status === "partial") {
...
export default function PublicationOrchestratorPanel({
  ...
  const [portals, setPortals] = useState<PortalSnapshot[]>([]);
  const [socialConnections, setSocialConnections] =
    useState<SocialConnection[]>([]);
  const [selectedPortals, setSelectedPortals] =
    useState<Set<PortalId>>(new Set());
  const [run, setRun] = useState<PublicationRun | null>(null);
  const [summary, setSummary] =
    useState<PublicationSummary>(EMPTY_SUMMARY);
  const [capabilities, setCapabilities] =
    useState<PublicationCapabilities>(DEFAULT_CAPABILITIES);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState<
    "prepare" | "dispatch" | "reconcile" | null
  >(null);
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");

  const applyPublicationResponse = useCallback(
    (data: PublicationResponse) => {
      if (data.run !== undefined) {
        const nextRun = data.run ?? null;
        setRun(nextRun);

        if (nextRun && onPublicationStateChange) {
          onPublicationStateChange({
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [runResponse, portalResponse, socialResponse] = await Promise.all([
        fetch(`/api/listings/${encodeURIComponent(listingId)}/publication-run`, {
          method: "GET",
          credentials: "include",
          cache: "no-store",
        }),
        market === "DE"
          ? fetch("/api/portal-connections/de", {
              method: "GET",
              credentials: "include",
              cache: "no-store",
            })
        ...
        fetch(
          process.env.NODE_ENV === "production"
            ? "/api/social-connections?environment=production"
      ...
      if (
        ...
        socialResponse.status === 401
      ) {
        window.location.href = "/login";
        return;
      }

      const runData = (await runResponse.json()) as PublicationResponse;
      ...
      applyPublicationResponse(runData);
      ...
      if (portalResponse) {
        ...
        const nextPortals: PortalSnapshot[] = Array.isArray(portalData.portals)
          ? portalData.portals
          : [];
        ...
        if (!runData.run) {
          setSelectedPortals(
            new Set(
              nextPortals
                .filter((portal) => isPortalReadyForControlledTest(portal))
      ...
      const socialData = await socialResponse.json().catch(() => null);

      if (
        socialResponse.ok &&
        socialData?.success &&
        Array.isArray(socialData.connections)
      ) {
        setSocialConnections(socialData.connections);
      } else {
        /*
         * Social ist noch nicht Teil des automatischen V1-Dispatchers.
         * Ein Fehler dort darf die Portalsteuerung nicht blockieren.
         */
  ...
  }, [listingId, market, applyPublicationResponse]);

  useEffect(() => {
    void load();
  }, [load]);

  const reconcileByRunId = useCallback(
    async (runId: string, silent = false) => {
      try {
        ...
        const response = await fetch(
          `/api/listings/${encodeURIComponent(listingId)}/publication-run`,
          {
            method: "POST",
            credentials: "include",
            cache: "no-store",
        ...
        applyPublicationResponse(data);

        if (!silent) {
          setMessage("Veröffentlichungsstatus wurde aktualisiert.");
        }
    ...
    [listingId, applyPublicationResponse]
  );

  const hasLinkedJob = Boolean(
    run?.targets.some((target) => target.portalJobId || target.socialJobId)
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void reconcileByRunId(run.id, true);
    }, 5000);

    return () => {
      window.clearInterval(timer);
    };
  }, [run, hasLinkedJob, reconcileByRunId]);

  const readyPortals = useMemo(
    () => portals.filter((portal) => isPortalReadyForControlledTest(portal)),
    [portals]
  );

  const verifiedSocialConnections = useMemo(
    () =>
  ...
  function togglePortal(portal: PortalId) {
    const snapshot = portals.find((candidate) => candidate.portal === portal);

    if (!snapshot || !isPortalReadyForControlledTest(snapshot)) {
      return;
    }
  ...
  async function prepareRun() {
    if (busy || selectedPortals.size === 0) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `/api/listings/${encodeURIComponent(listingId)}/publication-run`,
        {
          ...
          body: JSON.stringify({
            action: "prepare",
            targets: Array.from(selectedPortals).map((portal) => ({
              kind: "portal",
              portal,
            })),
          }),
      ...
      applyPublicationResponse(data);
      setMessage("Ziele vorbereitet. Es wurde noch nichts extern veröffentlicht.");
    } catch (prepareError) {
      setError(
  ...
  async function dispatchRun() {
    if (!run || busy || !capabilities.canDispatchPortals) {
      return;
    }
    ...
    <div className="mt-4 grid gap-3">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-black text-white">
              {run ? statusLabel(run.status) : "Ziele auswählen"}
            </p>
          </div>
      ...
      {!run ? (
        <>
          {market === "DE" ? (
            <div className="grid gap-2">
              ...
              {portals.length === 0 ? (
                ...
                portals.map((portal) => {
                  const ready = isPortalReadyForControlledTest(portal);
                  const readinessLabel = portalReadinessLabel(portal);
                  const readinessDetails = portalReadinessDetails(portal);
                  const selected = selectedPortals.has(portal.portal);
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        checked={selected}
                        disabled={!ready}
                        onChange={() => togglePortal(portal.portal)}
                        className="h-4 w-4"
                      />
                      ...
                      <span
                        title={readinessDetails}
                        className={
                          ready
                            ? "text-[9px] font-black text-emerald-300"
                            : portal.externalReadiness?.status === "BLOCKED"
          ...
          <div className="grid gap-2">
            ...
            {verifiedSocialConnections.length > 0 ? (
              verifiedSocialConnections.map((connection) => (
                <div
                  key={connection.id}
          ...
          <button
            type="button"
            disabled={busy !== null || selectedPortals.size === 0}
        ...
        <>
          ...
          {run.status !== "published" ? (
            <button
              type="button"
              disabled={busy !== null || !capabilities.canDispatchPortals}
          ...
          {!capabilities.canDispatchPortals && run.status !== "published" ? (
          ...
          {hasLinkedJob && run.status !== "published" ? (
            <button
              type="button"
              disabled={busy !== null}
              onClick={() => void reconcileByRunId(run.id)}
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2">
          ...
          {readyPortals.length === 0 ? (
            <div className="mt-1">
              <p className="text-[10px] font-black leading-4 text-white">
                Noch kein Portal verbunden
              </p>

              <a
                href="/cockpit#portale"
                className="mt-2 inline-flex min-h-8 items-center justify-center rounded-lg border border-amber-300/25 bg-amber-300/[0.08] px-2.5 py-1.5 text-[9px] font-black text-amber-200 no-underline transition hover:bg-amber-300/[0.14]"
              >
                Portale verbinden →
              </a>
            </div>
